...
class Fonctions3:
    ...
    !/usr/bin/env python
    coding: utf-8
    3e version du script de versement... ou 4e ?
    '''
    ...
    def __init__(self):
        ...
        self.nom_table_var = f'_{self.nom_schema}_var'.lower()
        self.nom_table_mod = f'_{self.nom_schema}_mod'.lower()
        self.nom_table_dict = f'_{self.nom_schema}_dict'.lower()
    ...
    def connexion_observatoire(self):
        ...
        while True:
            ...
            observatoire = input("Veuillez choisir l'observatoire auquel vous êtes rattaché : ")
            ...
            if observatoire in observatoires:
                ...
                try:
                    ...
                    print((f"Vous avez choisi l'observatoire {params['nom_obs']}. \n" 
                        f"Vous vous apprêtez actuellement à faire votre versement dans la base de données {params['database_pg']} et le schéma {params['nom_schema']}."))
    ...
    def traitement(self):
        ...
        input_ok = False
        while not input_ok:
            self.choix_input = input("""Souhaitez-vous saisir les libellés et catégorie de regroupement pour toutes vos nouvelles modalités ?\n(Répondez par : OUI ou NON)""").upper()
            if self.choix_input == 'NON' or self.choix_input == 'OUI':
                input_ok = True
                print(f"Vous avez répondu : {self.choix_input}")
                if self.choix_input == 'NON':
        ...
        self.id, self.id_versement = self.importe('val')
        self.nouvelles_mod = []
    ...
    def traitement_var(self, nom_feuilles):
        ...
        for nom_var in nom_feuilles:
            ...
            if(unidecode.unidecode(nom_var).lower() in self.dictionnaire_var.keys()):
                ...
                if self.choix_input:
                    ...
                    if not var_regroupement:
                        var_regroupement = nom_var
                else:
                    joli_nom_var = nom_var
                    lib_long_var = ''
                    var_regroupement = unidecode.unidecode(nom_var).lower()
                ...
                var_a_rajouter = {
                    ...
                    self.colnames_var[1] : unidecode.unidecode(nom_var).lower(),
                ...
                print(f'la variable {nom_var} à été insérée')
        ...
        return self.dictionnaire_var | dict_new_var
    ...
    def traitement_mod(self, feuille):      
        ...
        for nom_mod in feuille.columns[1:]: # 1: pour ne pas prendre en compte la première colonne qui sera code_entite
            ...
            if(nom_mod_propre in self.dictionnaire_mod.keys()):
                ...
                self.dictionnaire_mod = self.dictionnaire_mod | {nom_mod_propre: self.max_id_mod}
        ...
        for ligne in range(len(feuille)):
            ...
            for col, code_moda in enumerate(dict_mod_feuile.values()):
                ...
                new_row = {
                    ...
                    self.colnames_val[5]: self.dictionnaire_var[nom_feuille],
        ...
        self.livre = {
            ...
            'variable': {
                ...
                'dtype': {self.colnames_var[0]: int, self.colnames_var[1]: str, self.colnames_var[2]: str,
                          self.colnames_var[3]: str, self.colnames_var[4]: str}
            },
            'modalite': {
                ...
                'dtype': {self.colnames_mod[0]: int, self.colnames_mod[1]: str, self.colnames_mod[2]: str,
                          self.colnames_mod[3]: str, self.colnames_mod[4]: str}
            },
            'valeur': {
                'nom_table': self.nom_table,
                ...
                'dtype': {self.colnames_val[0]: int, self.colnames_val[1]: int, self.colnames_val[2]: int,
                          ...
                          self.colnames_val[6]: int, self.colnames_val[7]: float}
            }
        }
    ...
    def creation_table_pg(self, table):
        ...
        self.cur.execute(requete)
            # print(f"La table {self.livre[table]['nom_table']} à été créée avec succès dans pgAdmin \n")
    ...
    ############# insertion données
    ##############################################################################################################################
    def insertion_donnees2(self, table):
        """
        ...
        with open(self.livre[table]['chemin'], 'r', encoding="utf-8-sig") as file:
            ...
            for i,row in enumerate(reader):
                ...
                if current_time - start_time >= 1:  # Vérifie si une seconde s'est écoulée
                    seconde += 1
                    self.ligne_sec.append([seconde,i - ligne_moins1,i])
                    print(seconde)
                    print(i - ligne_moins1)
                    print(ligne_moins1)
                    ligne_moins1 = i
                ...
                if(len(values)==5):           # à adapter selon nbre de colonnes du fichier variables
                    self.cur.execute( "INSERT INTO {} VALUES (%s, %s, %s, %s, %s)".format(self.livre[table]['nom_table']),values)
                elif (len(values)==6):        # à adapter selon nbre de colonnes du fichier modalites
                    self.cur.execute( "INSERT INTO {} VALUES (%s, %s, %s, %s, %s, %s)".format(self.livre[table]['nom_table']),values)
                elif (len(values)==8):        # à adapter selon nbre de colonnes du fichier valeur
                    self.cur.execute( "INSERT INTO {} VALUES (%s, %s, %s, %s, %s, %s, %s, %s)".format(self.livre[table]['nom_table']),values)

        return self.ligne_sec
            # print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n')
            # print(self.styleP('la requête a été executée\n', color='green'))
            # print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')
        ...
        #     print("Les données importées ont été inserées avec succès dans la table {} \n".format(str(table)))
            ...
            # Pour améliorer les performances de l'insertion de données dans une base de données PostgreSQL à partir de Python, vous pouvez utiliser la méthode executemany() de l'objet curseur au lieu d'utiliser la méthode execute() dans une boucle.
    ...
    def insertion_donnees(self, table): #changement
        ...
        with open(self.livre[table]['chemin'], 'r', encoding="utf-8-sig") as file:
            ...
            if values: # Si la table est non vide.
                ...
                print("Le temps pour préparer le versement des données de la table {} à été de {:.3f} secondes".format(table, tps2 - tps1))
    ...
    # La méthode executemany() permet d'exécuter une requête SQL avec plusieurs ensembles de paramètres en une seule fois, ce qui peut réduire considérablement le temps d'exécution pour les grandes quantités de données.

    # Voici un exemple d'utilisation de la méthode executemany() pour insérer des données dans une table PostgreSQL :
    # import psycopg2

    # # Créer une connexion à la base de données
    # con = psycopg2.connect(database="mydatabase", user="myuser", password="mypassword", host="127.0.0.1", port="5432")
    ...
    # # Exécuter la requête SQL avec les données à l'aide de la méthode executemany()
    ...
    def liste_table(self, afficher):
        ...
        if not tables:
            print(self.styleP('Aucune table présente dans ce schéma \n', style='bold', color= "blue"))
        else:
            print(self.styleP(f'Tables existantes dans le schéma "{self.nom_schema}" sur PgAdmin: \n', style='bold', color= "blue")) 
            for table in tables:
    ...
    def liste_feuille(self, nom_excel):
        ...
        print(self.styleP('Variable(s) pour le jeu de données:  \n', style='bold', color="blue"))
    ...
    Quel dictionnaire voulez-vous ?\n Veuillez choisir le type de dictionnaire que vous souhaitez :
        ...
        # Connexion unique pour toute la fonction
        con = self.connexion()
        cur = con.cursor()
        ...
        cur.execute(f"""
            SELECT table_name
            FROM information_schema.tables
            WHERE table_schema = '{self.nom_schema}' AND
                (table_name LIKE '%mod' OR table_name LIKE '%var');
        """)
        tables = cur.fetchall()

        if not tables:
            print('Pas de données, pas de dict :/')
            return
        ...
        if self.nom_table not in (mod, var):
            # Construire la requête de self.base
            var_query = f"""
                SELECT distinct {self.nom_table}.id_var, {var}.nom_var, {var}.joli_nom_var
                FROM {self.nom_schema}.{self.nom_table}
                JOIN {self.nom_schema}.{var} ON {self.nom_table}.id_var={var}.id_var
            """
            
            mod_query = f"""select distinct 
            {self.nom_table}.id_mod, {mod}.joli_nom_mod, {mod}.nom_mod, {mod}.mod_regroupement, {mod}.lib_long_mod  
            from {self.nom_schema}.{self.nom_table} 
            join {self.nom_schema}.{mod} on {self.nom_table}.id_mod={mod}.id_mod"""

            base_query = f"""select distinct 
                    {self.nom_table}.id_mod, {mod}.joli_nom_mod, {mod}.nom_mod, {mod}.mod_regroupement, {mod}.lib_long_mod,
                    {self.nom_table}.id_var, {var}.nom_var,{var}.joli_nom_var, 
                    {var}.origine, {self.nom_table}.annee, {self.nom_table}.echelle  
                    from {self.nom_schema}.{self.nom_table} 
                    join {self.nom_schema}.{mod} on {self.nom_table}.id_mod={mod}.id_mod 
                    join {self.nom_schema}.{var} on {self.nom_table}.id_var={var}.id_var"""

            # Ajouter des conditions spécifiques basées sur 'quest'
            if quest == '2':
                theme = input(" Saisissez un mot clé pour une recherche plus ciblée dans le dictionnaire ").lower()
                additional_query = f" WHERE {var}.joli_nom_var LIKE '%{theme}%'"
                final_query = base_query + additional_query
            elif quest == '1':
                final_query = base_query
            elif quest == '3':
                final_query = mod_query
            elif quest == '4':
                final_query = var_query        
            try:
                cur.execute(final_query)
                result = pd.read_sql_query(final_query, con)
    ...
    def get_nom_col(self): #changement table,
        ...
        try:
            ...
            # SELECT DISTINCT _test_mod.joli_nom_mod, _test_mod.id_mod FROM test._test_mod, test.demog_popmen_insee WHERE _test_mod.id_mod= demog_popmen_insee.id_mod
            # self.theme + '_' + self.base + '_' + self.source 
            ...
            # def afficher_liste_par_ligne(liste, n, interval):
            ...
            # return [ (column_info[0], column_info[1], column_info[2]) for column_info in columns_info]   #column_info[0], column_info[1], column_info[2]
        except (Exception, psycopg2.Error) as error:
    ...
    def remplacerCaratereSpeciaux(self,feuille):
        """
        Donnée : feuille est le fichier (csv ou les sheets excel) original à traiter          
        Résultat : retourne feuille avec des noms de colonnes sans caractères spéciaux
        """
        
        # création de la liste des noms de colonnes de feuille
        listeCols = feuille.columns
        listeCols.tolist()
        ...
        for i in range(len(listeCols)):
            ...
            if(string1.isnumeric()):
                new_name = new_name.replace(string1, '_'+string1,1)  #modal
            if(('+') in new_name):
        ...
        feuille.columns = tabNewColonnes
    ...
    # def insertion_donnees2(self, table): #changement
    #     """
    #     Données : fichierCsv est un fichier CSV dont on veut reproduire la structure sur une self.base de données
    #             con = une instance de la fonction connexion
    #             table = nom (en string) de la table où vont être insérées les données
    ...
    def creationVueSchema(self): #changement
    ...
    #  rep = []
        ...
        if rep:
            print(self.styleP("Table trouvée :", color= 'green'))
            for row in rep:
                print(row[0])  
        else:
            print(self.styleP("Aucune table trouvée.", color= 'red'))
        ...
        for tab in range(len(rep)):
            ...
            try:
                ...
                for row in tqdm(result2):
                    ...
                    print(self.styleP("Construction de la vue sans indicateur pour l'échelle {}... \n", color= 'purple').format(self.echelle))
                    ...
                    print(self.styleP("La vue " + self.nom_schema+'.V_'+view_name_avec_echelle + " a été créée avec succès dans PostgreSQL \n", style='bold'))
        ...
        (Vous pourrez vous self.baser sur les indicateurs que vous allez créer ...)
        """)
        ...
        Choisissez un nom pour l\'indicateur à créer :  """ )
                    ...
                    try:
                        ...
                        for row in result2:
                            ...
                            print(self.styleP("Construction de la vue avec indicateur pour l'échelle {}... \n", color= 'purple').format(self.echelle))
                ...
                # try:
                            ...
                            cursor2.execute(f"""
                            INSERT INTO {self.nom_schema}.indicateurs_descriptions (nom_vue, description)
                            VALUES (%s, %s)
                            ON CONFLICT (nom_vue) DO UPDATE SET description = EXCLUDED.description;  
                            """, (self.nom_schema+'.I_'+view_name_avec_echelle, description))
                            self.con.commit()

                            print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n')
                            print(self.styleP("La vue " + self.nom_schema+'.I_'+view_name_avec_echelle + " à été créée avec succès dans PostgreSQL \n", style= 'bold', color= 'cyan'))
                            print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')

                    except (Exception) as error :
                        print ("Erreur lors de la création de la vue "+ self.nom_schema+'.I_'+view_name_avec_echelle +" dans PostgreSQL: ", error)
                        print(sqlQuery2)
    ...
    def description_vues(self):
        ...
        req = 'SELECT nom_vue, description FROM '  + self.nom_schema+'.indicateurs_descriptions '
    ...
    def create_vue_par_annee(self):
        ...
        Opérations terminées  """, color='green'))
                ...
                nom_vue = "" #input(""" Choisissez un nom pour l\'indicateur à créer :  \n""" )  
        ...
        """,val)
            ...
            L'entrée ne peut être vide.""", color='red'))
                ...
                while choix_nom2.strip() == "" :
                    choix_nom2 = input(""" 
            Veuillez saisir un nom pour la vue temporaire qui va être créée :  \n
            """ )
                    if choix_nom2.strip() == "":
                        print(self.styleP("""
            L'entrée ne peut être vide.""", color='red'))        
            ...
            """,(nom_vue,self.nom_schema))
                ...
                if "GROUP BY" in req1:
                    parts = req1.split("GROUP BY")
                    req2 = f"{parts[0]} WHERE t1.annee = '{choix_annee}'  GROUP BY {parts[1]}"
                else:
                    req2 = req1 + f"{req1} WHERE t1.annee = '{choix_annee}' "
                ...
                print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n')
                print(self.styleP("La nouvelle vue " + self.nom_schema+'.I_'+choix_nom2  + " à été créée avec succès dans PostgreSQL \n", style= 'bold', color= 'cyan'))
                print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')
        ...
        while True:
                ...
                while( not  OK):
                    ...
                    Le nom de l'indicateur ne peut pas être vide.""", color= 'red'))
                        ...
                        try:
                            ...
                            print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n')
                            print(self.styleP("La nouvelle vue " + self.nom_schema+'.I_'+choix_nom  + " à été créée avec succès dans PostgreSQL \n", style= 'bold', color= 'cyan'))
                            print('+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+')

                        except (Exception) as error :
                            print ("Erreur lors de la création de la vue "+ self.nom_schema+'.I_'+choix_nom  +" dans PostgreSQL: ", error)
                            print(sqlQuery2)
            ...
            ### ajout boucle while pour exceptions
                while nb_ind2 is None:
                    try:
                        saisie = input(" Entrez le nombre d'indicateurs à calculer : ")
                        if saisie.strip() == "":
                            print(self.styleP("""
                Veuillez entrer un nombre valide  """, color='red')) 
                        elif not saisie.isdigit():
                ...
                try:
                    ...
                    print ("Erreur lors de la création de la vue "+ self.nom_schema+'.I_'+choix_nom2  +" dans PostgreSQL: ", error)
                    print(sqlQuery2)
    ...
    def get_nom_col(self): #changement table,
        ...
        try:
            ...
            print("\n Liste des joli_nom_mod :\n")
            print(joli_nom_mod_l)
            # self.afficher_liste_par_ligne(joli_nom_mod_l, n=5, interval=40)  # n représente le nombre de mots qu'on souhaiterais afficher par ligne

            print("\n")

            print("\n Liste des lib_long_mod :\n")
            # self.afficher_liste_par_ligne(lib_long_mod_l, n=5, interval=40) 
